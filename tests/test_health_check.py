...
def test_ping(api_client):
    ...
    assert status_code == 201, f"Статус код не совпал с {status_code}"
...
def test_ping_wrong_http_method(api_client, mocker):
    ...
    with pytest.raises(Exception, match='Expected status code 201 but got 405'):
        api_client.ping()
...
def test_ping_internal_server_error(api_client, mocker):
    ...
    mocker.patch.object(api_client.session, 'get', return_value=mock_response)
    with pytest.raises(Exception, match='Expected status code 201 but got 404'):
        api_client.ping()
...
def test_ping_success_different_code(api_client, mocker):
    ...
    mocker.patch.object(api_client.session, 'get', return_value=mock_response)
    with pytest.raises(Exception, match='Expected status code 201 but got 200'):
        api_client.ping()
